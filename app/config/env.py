...
class Settings(BaseSettings):
    ...
    # Site settings
    SITE_URL: str = Field(
        default="http://localhost:8000", description="Base URL for the site"
    )
    SITE_NAME: str = Field(
        default="AI Micro-Businesses Assistant", description="Name of the site"
    )
    ...
    API_PREFIX: str = Field(
        default="ai-business-assistant/api/v1", description="API route prefix"
    )
    ...
    VL_MODEL: str = Field(
        default="", description="Model to use for vision-language tasks"
    )
    VL_BASE_URL: str = Field(
        default="https://openrouter.ai/api/v1",
        description="Base URL for vision-language LLM API",
    )
    VL_API_KEY: str = Field(default="", description="API key for vision-language LLM")
    VL_TEMPERATURE: float = Field(
        default=0.6, description="Temperature for vision-language LLM generation"
    )

    # Legacy OpenRouter settings (for backward compatibility)
    OPENROUTER_API_KEY: str = Field(
        default="", description="API key for OpenRouter (legacy)"
    )
    OPENROUTER_BASE_URL: str = Field(
        default="https://openrouter.ai/api/v1",
        description="Base URL for OpenRouter API (legacy)",
    )
    OPENROUTER_MODEL: str = Field(
        default="", description="Model to use with OpenRouter (legacy)"
    )
    ...
    # Toolbox settings
    TOOLBOX_BASE_URL: str = Field(
        default="http://0.0.0.0:5000", description="Base URL for the toolbox service"
    )
    ...
    GOOGLE_GEN_AI_API_KEY: str = Field(
        default="", description="API key for Google Gen AI"
    )
