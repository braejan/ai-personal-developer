...
def _get_prompt_template(template: str, values: TypedDict = None) -> str:
    if values is None:
        values = {}
    all_vars = {
...
def get_prompt(template_name: str = None, values: TypedDict = None) -> str:
    if values is None:
        values = {}
    if not template_name:
        return _get_prompt_template(default_prompt, values)
    current_dir = os.path.dirname(__file__)
    template = _read_prompt_template_file(os.path.join(current_dir, f"{template_name}.md"))
